...
VehicleModel load_vehicle_csv(const std::filesystem::path& path) {
    ...
    v.control_fin_count = get(values, "control_fin_count", v.control_fin_count);
    ...
    v.control_roll_target_deg = get(values, "control_roll_target_deg", v.control_roll_target_deg);
    v.control_pitch_target_deg = get(values, "control_pitch_target_deg", v.control_pitch_target_deg);
    v.control_yaw_target_deg = get(values, "control_yaw_target_deg", v.control_yaw_target_deg);
    v.control_roll_kp = get(values, "control_roll_kp", v.control_roll_kp);
    v.control_roll_kd = get(values, "control_roll_kd", v.control_roll_kd);
...
void write_trajectory_csv(const std::filesystem::path& path, const SimulationResult& result, const VehicleModel& vehicle) {
    std::ofstream out(path);
    out << "time_s,north_m,east_m,down_m,altitude_m,lat_deg,lon_deg,alt_m,vn_mps,ve_mps,vd_mps,thrust_n,wind_n_mps,wind_e_mps,wind_d_mps,cp_from_nose_m,control_roll_deflection_deg,control_pitch_deflection_deg,control_yaw_deflection_deg,control_moment_x_nm,control_moment_y_nm,control_moment_z_nm\n";
    ...
    for (const auto& p : result.points) {
        ...
        out << p.state.t_s << ',' << p.state.position_ned_m.x << ',' << p.state.position_ned_m.y << ',' << p.state.position_ned_m.z << ','
            ...
            << p.control_roll_deflection_deg << ',' << p.control_pitch_deflection_deg << ',' << p.control_yaw_deflection_deg << ','
...
void write_graph_svgs(const std::filesystem::path& directory, const SimulationResult& result, const std::vector<DispersionPoint>& dispersion) {
    ...
    std::vector<Series> control{
        {"Roll fins [deg]", "#4fc3f7", {}},
        {"Pitch fins [deg]", "#ffca28", {}},
        {"Yaw fins [deg]", "#66bb6a", {}},
    ...
    for (const auto& p : result.points) {
        ...
        control[0].points.push_back({p.state.t_s, p.control_roll_deflection_deg});
        control[1].points.push_back({p.state.t_s, p.control_pitch_deflection_deg});
        control[2].points.push_back({p.state.t_s, p.control_yaw_deflection_deg});
        control[3].points.push_back({p.state.t_s, norm(p.control_moment_body_nm)});
    }
